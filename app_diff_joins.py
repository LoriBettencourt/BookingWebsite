...
def show_venue(venue_id):
    ...
    show_data = venue_data.shows
    print("show_data_beta: ", show_data)
    ...
    for show in show_data:
        ...
        if show.show_date_time < datetime.now():
            past_show_list.append(show_info)
        else:
            future_show_list.append(show_info)
    
    print("test print past show list:", past_show_list)
    print("test print future show list:", future_show_list)
    ...
    # past_show_data = db.session.query(Show).join(Artist).filter(
    #     Show.venue_id==venue_id,Show.show_date_time < datetime.now())
    # print("past_show_data: ",past_show_data)
    # future_show_data = db.session.query(Show).join(Artist).filter(
    #     Show.venue_id==venue_id,Show.show_date_time > datetime.now())
    # print("future_show_data: ", future_show_data)
...
def show_artist(artist_id):
    ...
    past_show_data = db.session.query(Show).join(Venue).filter(
        Show.artist_id==artist_id).filter(Show.show_date_time>datetime.now()).all()
    future_show_data = db.session.query(Show).join(Venue).filter(
        Show.artist_id==artist_id).filter(Show.show_date_time>datetime.now()).all()
